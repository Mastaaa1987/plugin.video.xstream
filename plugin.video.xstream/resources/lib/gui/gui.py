...
#kasi
 ...
 itemValues['plot'] = '\xc2\xa0' #kasi Alt 255
 except:
 pass
 ...
 'System.HasAddon(%s)' % 'script.module.xstream.trailer'): # Schauen ob Addon installiert
 ...
 contextmenus += [(contextitem.getTitle(),
 "RunPlugin(plugin://script.module.xstream.trailer/?action=play&name=%s&url=&language=)" % (
 itemValues['title'],),)]
 contextitem.setTitle(cConfig().getLocalizedString(30244)) # Playlist hinzuf\xc3\xbcgen
 ...
 iSeconds = iSeconds plug plug2 plug3 sites sites0 test.py test.sh test.txt xstream 1000
...
class cGui:
 ...
 self.pluginPath = ''
 self.isMetaOn = cConfig().getSetting('TMDBMETA\
